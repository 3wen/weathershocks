...
This page presents the Dynare codes for the two simplified models presented in [Chapter -@sec-dsge-theory]:

- The RBC model in @sec-dsge-rbc,
- The NK model with price rigidities in @sec-dsge-nk.

The models are solved in Dynare (version $\geq$6, in a MATLAB environment).
...
The codes to replicate the results of the paper are in the `script/dsge` folder of the [weathershocks Github repository](https://github.com/3wen/weathershocks/tree/main/scripts/dsge).
...
- `RBC_q0.mod`: Estimated model **without weather shocks**,
- `RBC_q1.mod`: Baseline estimated model **with weather shocks**,
- `COMPARE.mod`: Script comparing model fit across specifications,
- `dataNZ_cubic_trend.m`: Input dataset (New Zealand macroeconomic data).
...
## Simplified Two-Sector Weather RBC Mode {#sec-dsge-rbc}
...
## Two Sector New Keynesian Model with Weather Shocks and Land {#sec-dsge-nk}
