...
spei_quarterly_mean <-
  ...
  group_by(cell_id, year, quarter) |>
  summarise(
    across(starts_with("SPEI_"), 
      ~ mean(.x, na.rm = TRUE), .names = "{.col}_qmean"
    ),
    .groups = "drop"
  )
...
To ensure comparability across months, we construct one map layer per month and then bind them into a single `sf` object for faceting.
```{r define-data_map}
# For region layers
