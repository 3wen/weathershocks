...
```{r look-region-names-agri}
sort(unique(tb_agri_shares$region))
```

We notice two types of issues here:

1. Spelling differences (easy to correct): Manawatu-W*h*anganui vs. Manawatu-Wanganui
2. Different spatial aggregation: _Tasman_ and _Nelson_ are merged into a single region in the agricultural data (see @fig-map-issue-tasman).

For the first issue, the fix is very easy, we just need to adjust the spelling.
```{r rename-region-Manawatu-Whanganui}
...
```{r}
#| fig-cap: Regions merged in the agricultural data.
#| label: fig-map-issue-tasman
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
...
Now we duplicate each `"Tasman/Nelson"`{.R} row into two rows (`"Tasman"`{.R} and `Nelson`{.R}) and allocate `gdp_a` and `agri_share_region` using the weights:
```{r redefine-tb_agri_shares-tasman-nelson}
...
```{r sanity-check-weather-region-names}
unique(region_weather_monthly$region)[! unique(region_weather_monthly$region) %in% unique(tb_agri_shares$region)] |> 
  cat(sep = "\n")
```
...
```{r sanity-check-agri-region-names-not-in-map}
...
```{r define-national_weather_monthly}
...
```{r}
#| fig-cap: Precipitation in New Zealand on a monthly basis.
#| label: fig-precip-nz-monthly
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
...
```{r}
#| fig-cap: Precipitation in New Zealand on a quarterly basis.
#| label: fig-precip-nz-quarterly
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
...
```{r}
#| fig-cap: Precipitation in New Zealand on a monthly basis (1987--2024).
#| label: fig-precip-nz-monthly-highlight-2013
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
  data = national_weather_monthly |> 
...
```{r}
#| fig-cap: Barplot of precipitation in New Zealand on a monthly basis (1987--2024).
#| label: fig-precip-nz-monthly-barplot
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
  data = national_weather_monthly |> 
...
```{r}
#| fig-cap: Soil Moisture Deficit Index in New Zealand, monthly basis.
#| label: fig-SMDI-nz-monthly
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
  data = national_weather_monthly |> 
...
```{r}
#| fig-cap: Soil Moisture Deficit Index in New Zealand, quarterly basis.
#| label: fig-SMDI-nz-quarterly
#| code-fold: true
#| code-summary: Codes to create the Figure.
ggplot(
  data = national_weather_quarterly |> 
...
Table: Dictionary of variables for monthly precipitation (`national_weather_monthly`) {#tbl-dict-monthly}
...
Table: Dictionary of variables for quarterly precipitation (`national_weather_quarterly`) {#tbl-dict-monthly}
